...
class Sequence(BTNode):
	...
	def execute(self, delta = 0):
		...
		numKids = BTNode.getNumChildren(self)
		if numKids == 0:
			return True
		yuvanesh = BTNode.getCurrentIndex(self)
		if yuvanesh <= numKids - 1:
			curChild = BTNode.getChild(self, yuvanesh)
			powerMan = curChild.execute()
			if powerMan == False:
				return False
			if powerMan == None:
				return None
			tharun = yuvanesh + 1
			BTNode.setCurrentIndex(self, tharun)
			if tharun <= numKids - 1:
				return self.execute()
