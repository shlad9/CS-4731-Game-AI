...
def myBuildTree(agent):
	...
	retreat = makeNode(Retreat, agent)
	hpDam = makeNode(HitpointDaemon, agent)
	sel2 = makeNode(Selector, agent)
	Enemy = makeNode(KillVisible, agent)
	attackM = makeNode(KillMinion, agent)
	attackH = makeNode(KillHero, agent)
	move = makeNode(ChaseMyMinion, agent)
	sel3 = makeNode(Selector, agent)
	...
	sel2.addChild(Enemy)
	sel2.addChild(sel3)
	sel3.addChild(attackH)
	sel3.addChild(attackM)
	Enemy.addChild(move)

	# root = makeNode(Selector, agent)
	#
	#
	# seq1 = makeNode(Sequence, agent)
	# seq2 = makeNode(Sequence, agent)
	# seq3 = makeNode(Sequence, agent)
	#
	# root.addChild(seq1)
	#
	# buff = makeNode(BuffDaemon, agent)
	# root.addChild(buff)
	# buff.addChild(seq2)
	#
	# retreat1 = makeNode(Retreat, agent, 50)
	# followMine = makeNode(ChaseMyMinion, agent)
	# sightDaemon = makeNode(KillVisible, agent)
	#
	# seq1.addChild(retreat1)
	# seq1.addChild(followMine)
	# seq1.addChild(sightDaemon)
	#
	# killenemy = makeNode(KillMinion, agent)
	# sightDaemon.addChild(killenemy)
	#
	# reatreat2 = makeNode(Retreat, agent, 50)
	# chaseHero = makeNode(ChaseHero, agent)
	# killHero = makeNode(KillHero, agent)
	#
	# seq2.addChild(reatreat2)
	# seq2.addChild(seq3)
	# seq3.addChild(chaseHero)
	# seq3.addChild(killHero)
...
class KillMinion(BTNode):
	...
	def execute(self, delta = 0):
		...
		if self.target == None or distance(self.agent.getLocation(), self.target.getLocation()) > BIGBULLETRANGE:
			...
			if isinstance(self.target, Hero) and self.agent.candodge:
				self.agent.dodge()
			self.shootAtTarget()
			return None
		return ret
...
class KillVisible(BTNode):


	def parseArgs(self, args):
		BTNode.parseArgs(self, args)
		self.radius = BIGBULLETRANGE
	...
	def execute(self, delta=0):
		...
		for enemy in enemies:
			if distance(enemy.getLocation(), self.agent.getLocation()) < self.radius:
				# Check failed
				print("exec", self.id, "fail")
				return False
		# Check didn't fail, return child's status
...
class ChaseMyMinion(BTNode):
	...
	def enter(self):
		BTNode.enter(self)
		self.timer = 50
		enemies = self.agent.world.getNPCsForTeam(self.agent.getTeam())
		if len(enemies) > 0:
			best = None
			dist = 0
			for e in enemies:
