...
public class MyLevelGenerator{
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		String reposit = "921867382918372819383728173";
		String repositTT = "22222333333999999111111777722222888833333366665555555";
        
        for (int i = 0; i < 100; i++) {
            base += bank.charAt(r.nextInt(bank.length()));
        }

        for (int i = 0; i < 55; i++) {
            base += reposit.charAt(r.nextInt(bank.length()));
        }

        for (int i = 0; i < 40; i++) {
            base += repositTT.charAt(r.nextInt(bank.length()));
        }

        individual.setChromosome(base);
	...
	private MyDNA pickIndividualForCrossover (ArrayList<MyDNA> population, MyDNA excludeMe)
	{
		...
		int thePick = rand.nextInt(population.size()) + 1;
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
        ...
        // YOUR CODE GOES BELOW HERE
        // TODO
        ArrayList<MyDNA> compilation = new ArrayList<MyDNA>();
        compilation.addAll(oldPopulation);
        compilation.addAll(newPopulation);

        for (int i = 0; i < this.getPopulationSize(); i++) {
                MyDNA bestInd = getBestIndividual(compilation);
                finalPopulation.add(bestInd);
        }
